...
% load data
folder='/home/sbarc/wrf/wrf401/sundowners/swex-0/run-64452560-z0-1way/';
inputfile=strcat(folder,'wrfout_swex-0_vertProfs_hovmoller_1way.mat');
load(inputfile)

runsLeg='default';
startComp=datenum(2018,4,28,6,0,0); % PST time
endComp=datenum(2018,4,29,12,0,0); % PST time
...
load(strcat('/home/sbarc/wrf/wrf401/sundowners/swex-0/westEastLocations.mat'));
...
hold on; box on
[c2,h2]=contour(lonsHovMul,timesHovMul,squeeze(Whovm.V10(j,lonsRange,:)),[-20 15.6 -13.4],...
    'k','LineWidth',2,'ShowText','on');
text(-119.45,datenum(2018,4,28,19,25,0),'sunset','color','k')
line(lonsHovMul(:,1),[repmat(datenum(2018,4,28,18,40,0),[length(lonsHovMul(:,1)),1])],...
    'LineWidth',2,'LineStyle','--','color','k')
text(-119.45,datenum(2018,4,29,5,55,0),'sunrise','color','k')
line(lonsHovMul(:,1),[repmat(datenum(2018,4,29,5,10,0),[length(lonsHovMul(:,1)),1])],...
    'LineWidth',2,'LineStyle','--','color','k')

% plot properties
ylabel('Time in April 2018 [dd/HH PST]');
...
Ti=title(strcat('(a) Western regime - V-component halfway leeslope [$m s^{-1}$]'),...
