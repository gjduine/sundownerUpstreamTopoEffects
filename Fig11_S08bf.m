% script to plot figure 11 and S8 b/f of manuscript on influence of terrain
...
for st=[10] % loop over locations --> upvalley of KSBA, MTIC and RHWC1
    for r=1:length(runs)
        st
        runX=runs{r}
...
%         ribPlot=squeeze((WRF.(runX).PBLH_riB_st(st,indSt:indEnd)+WRF.(runX).hgt_st(st)));
        ...
        clims2=[-0.002 0.03];
        c1=contourf(sizeTime,sizeHeight,real(BVplot),[clims2(1):0.002:clims2(2)],'edgecolor','none'); % first fill color
        hold on
        cmap2=flipud(hot(30));
        colormap(ax1(c),cmap2);   caxis(clims2);   set(ax1(c),'CLim',clims2)
        
        if c==2
            cb=colorbar;
            set(cb,'YLim',[clims2],'TickDir','out')
            ylabel(cb,'N [s^{-1}]')
        end
...
figName=strcat(regimeFileName{regX},'_cd');
