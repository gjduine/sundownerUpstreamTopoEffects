...
runpath(1,:) = strcat(folder,'/geo_em_toChange.d01.nc');
runpath(2,:) = strcat(folder,'/geo_em_toChange.d02.nc');
runpath(3,:) = strcat(folder,'/geo_em_toChange.d03.nc');
runpath(4,:) = strcat(folder,'/geo_em_toChange.d04.nc');
...
perc=0.3; % to what elevation do we change.
...
for d=[1:4]
    ...
    if (d==3 || d==4)
        ...
        %percEast=logspace(perc.^perc,1,jlength)/10; % have to apply the
        %square for good scaling in the beginning. Works great for 0.75 and
        %0.1  approaches
        percEast=linspace(perc,1,jlength); % a linear approach. Works too.
        j=1;
...
% plotting stuff
% station locations
stations_obs={'RHWC1','ElCap' , 'KSBA', 'SBVC1', 'MTIC1',  'Carp' ,'gap', 'oxn','la west','la east'};
lons_obs=[   -120.075 -119.965 -119.844 -119.706 -119.649 -119.515 -119.3 -119   -118.2   -117.8];  
lats_obs=[    34.517    34.458   34.426   34.456  34.461   34.406  34.406 34.406 34.406    34.406];
% % 
...
WRF.d01.lons_gc_st=[86 87 87 87 88 88 89 90 93 94];%WRF.d01.lons_gc_stInt;
WRF.d01.lats_gc_st=[84 83 83 83 83 83 83 83 83 83];%WRF.d01.lats_gc_stInt;
WRF.d02.lons_gc_st=[89 91 92 93 94 95 98 101 110 114];%WRF.d02.lons_gc_stInt;
WRF.d02.lats_gc_st=[79 78 77 78 78 77 77 77 77 77];%WRF.d02.lats_gc_stInt;
WRF.d03.lons_gc_st=[105 109 113 117 119 123 130 140 166 179];%WRF.d03.lons_gc_stInt;
WRF.d03.lats_gc_st=[103 100 99 100 101 98 98 98 98 98];%WRF.d03.lats_gc_stInt;
WRF.d04.lons_gc_st=[114 125 137 150 156 168 189 218 NaN NaN];%WRF.d04.lons_gc_stInt;
WRF.d04.lats_gc_st=[118 111 108 111 112 105 105 105 NaN NaN];%WRF.d04.lats_gc_stInt;
...
%     end
    ...
    for st=1:8%length(stations_obs)
...
fig=figGD('on');
pl=1;
for st=9:length(stations_obs)
    subplot_tight(1,2,pl,[0.25 0.05])
    hold on
    for d=[1:3]
        doX=strcat('d0',num2str(d));
        l3(d)=line(WRF.(doX).Lats(WRF.(doX).lons_gc_st(st),:),...
...
export_fig('sbareg_d01-d03_terrain_elev_mod_cx_sect_gaussianApproach_oldNew_withNorth','-png')
