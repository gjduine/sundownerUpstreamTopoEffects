...
% for eastern regime we use locations 1 and 3 for refugio and montecito
    ...
    text(-120.25,34.56,'Refugio','rotation',-20,'color','b')
    ...
    for f=1:length(tnumHourly)
        ...
        legStr3(f)=tnumHourly(f);
        legStr4(f)=tnumPDT{tr,f}(1); % strings for legend
    end
    ...
    set(gca,'CLim',[0 2.0223e+03],'XLim',[-121 -118.8],'YLim',[34.2 35.2],...
        'YTick',[34.2 34.4 34.6 34.8 35.0 35.2],'Layer','top')
    line(traj_lon{tr,1}(1),traj_lat{tr,1}(1),'LineStyle','none','Marker','x',...
    ...
    for f=1:length(tnumHourly)
        tr=1; % refugio
        p1(f)=plot(traj_hour{tr,f},traj_pressure{tr,f},'LineWidth',3,...
            'color',cmap1(f,:),'LineStyle','-','Marker',markers{f},'MarkerSize',10);
        legStr1(f)=tnumPDT{tr,f}(1);
