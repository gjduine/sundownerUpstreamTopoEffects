% script to plot figure 11 and S8 a/e of manuscript on influence of terrain
...
for st=[8]%[2,5,8] % loop over locations --> upvalley of KSBA, MTIC and RHWC1
    for r=1:length(runs)
        st
        runX=runs{r}
...
%         ribPlot=squeeze((WRF.(runX).PBLH_riB_st(st,indSt:indEnd)+WRF.(runX).hgt_st(st)));
        ...
        clims1=[-23 23];
        hold on
        c1=contourf(sizeTime,sizeHeight,Vplot,[-23:1:23],'edgecolor','none'); % first fill color
        
        bwr=flipud(bluewhitered(30));
        colormap(ax1(c),bwr);caxis([-23 23]);set(gca,'CLim',[-23 23])
        bwr=flipud(bluewhitered(30));
        colormap(ax1(c),bwr);caxis([-23 23]);set(gca,'CLim',[-23 23]) % for some reason this has to be done twice
        if c==2
            cb=colorbar;
...
figName=strcat(regimeFileName{regX},'_ab');
