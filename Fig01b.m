% script to plot figure 1b of manuscript on influence of terrain
...
cmapsea=[1 1 1];
clevs=[    1         200         400         800        1200        1600        2000];
for g=1
geoX=strcat('geo',num2str(g));
...
%     colormap(flipud(pink))
    ...
    p1=contourf(squeeze(WRF.(geoX).(doX).Lons),squeeze(WRF.(geoX).(doX).Lats),squeeze(WRF.(geoX).(doX).HGT),clevs);
    hold on
    p2=contour(squeeze(WRF.(geoX).(doX).Lons),squeeze(WRF.(geoX).(doX).Lats),squeeze(WRF.(geoX).(doX).HGT),...
        [0:200:1600],'color','k','LineWidth',2);
...
%     annotation('arrow',x2,y2,'LineWidth',3,'HeadWidth',15)
    ...
    PO=text(-120.49,34.32,'Pacific Ocean','color','k');

    set(gca,'CLim',[0 1600],'Layer','top')
    set(gca,'XLim',[-120.5 -119.3],'YLim',[34.3 34.65],...
        'XTick',[-120.25 -120.0 -119.75 -119.5 -119.25],...
        'YTick',[34.3 34.4 34.5 34.6])
    
%     % cross section line fig. 4/5
%     line([-120.075331 -120.075331],[34.4 34.62],'color','k','LineWidth',3)
%     textR=text(-120.104,34.395,'R','color','k','FontWeight','bold');
%     textRR=text(-120.106,34.625,'R`','color','k','FontWeight','bold');

%     line([-119.8436 -119.8436],[34.4 34.62],'color','k','LineWidth',3)
%     textC=text(-119.870,34.395,'C','color','k','FontWeight','bold');
%     textCC=text(-119.864,34.62,'C`','color','k','FontWeight','bold');

%     line([-119.649014 -119.649014],[34.4 34.62],'color','k','LineWidth',3)
%     textM=text(-119.675,34.4,'M','color','k','FontWeight','bold');
%     textMM=text(-119.677,34.62,'M`','color','k','FontWeight','bold');
...
%     textBB=text(-119.41,34.415,'B`','color','k','FontWeight','bold');
    ...
    p1(st)=plot(lons_obs(st),lats_obs(st),'LineStyle','none','Marker','x','color','w','LineWidth',5,...
        'MarkerFaceColor','b','MarkerSize',20);
    text1=text(-120.06,34.51,'Refugio (R)','rotation',-20,'color','w','FontWeight','bold');
    
    st=6; % montecito station
    p2(st)=plot(lons_obs(st),lats_obs(st),'LineStyle','none','Marker','x','color','w','LineWidth',5,...
        'MarkerFaceColor','r','MarkerSize',20);
    text2=text(-119.63,34.45,'Montecito (M)','rotation',-20,'color','w','FontWeight','bold');
    
%     cFG=[1 1 1];%[0.15 0.5 0.15];
%     st=9; % KSBA slopes
...
%     text3=text(-119.75,34.435,'SBVC1','rotation',0,'color',FG,'FontWeight','bold');
    ...
    p4(st)=plot(lons_obs(st),lats_obs(st),'LineStyle','none','Marker','x','color','w','LineWidth',5,...
        'MarkerFaceColor','b','MarkerSize',20);
    text4=text(-120.06,34.615,'Refugio upvalley','rotation',10,'color','w','FontWeight','bold');
    
    st=8; % north of SYM for Montecito
    p2(st)=plot(lons_obs(st),lats_obs(st),'LineStyle','none','Marker','x','color','w','LineWidth',5,...
        'MarkerFaceColor','r','MarkerSize',20);
    text2a=text(-119.63,34.54,'Montecito upvalley','rotation',10,'color','k','FontWeight','bold');
